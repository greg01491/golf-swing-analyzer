...
### 4.5b Ball detection & ball-anchored swing events (planned — see docs/ball-tracking-plan.md)
- FR25: The system shall auto-detect the golf ball at address in the down-the-line camera by searching a small region anchored on the golfer's tracked wrists (2D pose), not the whole frame. Rationale (proven on real footage): the room is full of round bright distractors, so a wrist-anchored ROI is required; and a plain white ball is too low-contrast against a green mat in indoor lighting (measured ball HSV ≈ [44,49,117] vs mat ≈ [44,45,112]), so a **bright coloured ball (orange/yellow) is required** for reliable hue-based detection.
- FR26: Given the detected ball, the system shall anchor **address (P1)** as the frame where the ball is present next to the club head and still, and **impact (P7)** as the frame where the ball disappears (is struck) — replacing/refining the current audio-trigger + geometry anchors, which are approximate (observed: club still far from the ball at the detected P7).
- FR27 (club head proxy): Since the club isn't a pose keypoint and can't be tracked through a fast swing (NG2), the system may estimate the club-head position at address only, as the ball location (club head sits at the ball at address) and/or the far end of the shaft line extrapolated from the wrists.
