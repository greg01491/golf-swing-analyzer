...
- [ ] Redo camera_2's intrinsics captures and the extrinsics (practice-swing) capture now that its rotation is fixed, recompute via the wizard, and confirm `mean_reprojection_error_px` is reasonable (well under ~5px) — **needs your rig**; the existing `Calib_rig.toml` (~79,000px reprojection error) is unusable and was computed before the rotation fix
