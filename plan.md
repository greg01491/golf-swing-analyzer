...
- Integrate chosen open-source pose model (spike/decision needed — see Risks) to extract 2D landmarks per frame, per camera.
...
- Build calibration workflow (e.g. checkerboard capture from both cameras) to compute relative camera pose.
- Implement triangulation of synced 2D landmark pairs into 3D landmark sequences.
- **Exit criteria:** a captured swing produces a 3D landmark sequence that visibly matches the recorded motion when plotted/played back.
...
|---|---|---|
| Pose model choice needs a spike — accuracy/speed tradeoffs unknown | Delays Phase 3/4 | Time-box a short spike in Phase 3 comparing 1-2 candidate open-source models before committing |
| Camera sync drift (two independent USB cameras) | Bad triangulation | Establish a sync method early (e.g. shared capture timestamp, or a clap/flash sync test) in Phase 1 |
...
- Spike A (Phase 3): candidate pose model shortlist + quick accuracy/speed comparison on sample footage.
